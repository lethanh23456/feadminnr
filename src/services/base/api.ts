...
export async function adminlogin(payload: { username?: string; password?: string }) {
	return axios.post(`${ipNR}/auth/login`, { ...payload });
}

export async function verifyOtp(payload: { otp: string; sessionId: string }) {
	return axios.post(`${ipNR}/auth/verify-otp`, payload);
}
